...
- Validation is a deliberate two-step exercise, not an auto-generated number. **Step 1** prints a deterministic held-out sample (15 abstracts) with the model's prediction; **Step 2** holds a `GROUND_TRUTH` dict keyed by PMID. The notebook ships with my own hand-labels (14 of 15 — one immunopathogenesis review was excluded as not fitting any theme cleanly), which is how it reaches ~64% zero-shot accuracy. To make the validation your own, re-label the abstracts yourself and re-run Step 2 for a fresh accuracy + per-class F1. Labels are keyed by PMID (not row order), so the score stays correct even though PubMed's relevance ordering shifts between runs. Any PMID you leave out is simply excluded from the metric — no fabricated numbers.
